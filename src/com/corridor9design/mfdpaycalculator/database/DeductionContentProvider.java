...
public class DeductionContentProvider extends ContentProvider {
	...
	private static final String AUTHORITY = "com.corridor9design.mfdpaycalculator.deduction.contentprovider";
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs){
		...
		case DEDUCTION_ID:
			String id = uri.getLastPathSegment();
			if (TextUtils.isEmpty(selection)){
				rowsUpdated = db.update(Deduction.TABLE_DEDUCTION, values, Deduction.TABLE_DEDUCTION + "=" + id + " and " + selection, null);
			} else {
			rowsUpdated = db.update(Deduction.TABLE_DEDUCTION, values, Deduction.TABLE_DEDUCTION + "=" + id + " and " + selection, selectionArgs);
			}
			break;
		default:
			throw new IllegalArgumentException("Unknown URI: " + uri);
